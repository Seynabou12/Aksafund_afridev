            ...
            <tr>
                ...
                <td class="actions">
                    ...
                    <?php if($user->status == 0){ ?>
                        <?= $this->Form->postLink(__('<i class="fas fa-lock "></i>'), ['controller' => 'Users', 'action' => 'delete', $user->id], ['escape' => false, 'class' => 'btn btn-sm btn-sm1 btn-danger', 'confirm' => __('Voulez vous désactiver l\'user n° :  {0}?', $user->id)]) ?>

                    <?php } else{?>
                        <?= $this->Form->postLink(__('<i class="fas fa-unlock "></i>'), ['controller' => 'Users', 'action' => 'delete', $user->id], ['escape' => false, 'class' => 'btn btn-sm btn-sm1 btn-danger', 'confirm' => __('Voulez vous activer n° :  {0}?', $user->id)]) ?>
                    <?php } ?>
                </td>
            </tr>
            <?php endforeach; ?>
